...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Send notification email to Dalapathi Sai Varma
...
${escapeHtml(cleanMessage)}
    ...
    // Try sending visitor acknowledgement email
    try {
      ...
      console.warn('Visitor acknowledgement skipped:', visitorErr?.message);
    }

    return res.status(200).json({
      success: true,
      message: `Thanks ${cleanName}! Your message has been sent successfully. I will get back to you soon.`,
      id: data?.id,
    });
  } catch (err) {
